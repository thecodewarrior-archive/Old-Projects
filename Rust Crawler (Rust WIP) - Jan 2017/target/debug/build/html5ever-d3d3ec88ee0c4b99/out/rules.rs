...
NoPush , Push , TreeBuilderActions
} ; use tree_builder :: interface :: {
TreeSink , Quirks , AppendNode
} ; use tokenizer :: {
Attribute , EndTag , StartTag , Tag
} ; use tokenizer :: states :: {
Rcdata , Rawtext , ScriptData , Plaintext
} ; use QualName ; use util :: str :: is_ascii_whitespace ; use std :: ascii :: AsciiExt ; use std :: mem :: replace ; use std :: borrow :: Cow :: Borrowed ; use std :: borrow :: ToOwned ; use tendril :: {
StrTendril , SliceExt
...
let elem = self . sink . create_element ( qualname ! ( html , "script" ) , tag . attrs ) ; if self . is_fragment ( ) {
...
let top = self . html_elem ( ) ; self . sink . add_attrs_if_missing ( top , tag . attrs ) ;
} Done
} :: tree_builder :: types :: TagToken ( :: tokenizer :: Tag {
kind : :: tokenizer :: StartTag , name : local_name ! ( "base" ) , ..
} ) | :: tree_builder :: types :: TagToken ( :: tokenizer :: Tag {
...
self . unexpected ( & tag ) ; match self . body_elem ( ) {
Some ( ref node ) if self . open_elems . len ( ) != 1 && ! self . in_html_elem_named ( local_name ! ( "template" ) ) => {
self . frameset_ok = false ; self . sink . add_attrs_if_missing ( node . clone ( ) , tag . attrs )
} , _ => {
}
} Done
} :: tree_builder :: types :: TagToken ( tag @ :: tokenizer :: Tag {
...
None => return Done , Some ( x ) => x ,
} ; self . sink . remove_from_parent ( body ) ; self . open_elems . truncate ( 1 ) ; self . insert_element_for ( tag ) ; self . mode = InFrameset ; Done
} EOFToken => {
...
self . close_p_element_in_button_scope ( ) ; if self . current_node_named ( local_name ! ( "menuitem" ) ) {
self . pop ( ) ;
...
declare_tag_set ! ( close_list = "li" ) ; declare_tag_set ! ( close_defn = "dd" "dt" ) ; declare_tag_set ! ( extra_special = [ special_tag ] - "address" "div" "p" ) ; let can_close : fn ( QualName ) -> bool = match tag . name {
local_name ! ( "li" ) => close_list , local_name ! ( "dd" ) | local_name ! ( "dt" ) => close_defn , _ => unreachable ! ( ) ,
} ; self . frameset_ok = false ; let mut to_close = None ; for node in self . open_elems . iter ( ) . rev ( ) {
let name = self . sink . elem_name ( node . clone ( ) ) ; if can_close ( name . clone ( ) ) {
to_close = Some ( name . local ) ; break ;
...
} self . generate_implied_end ( cursory_implied_end ) ; let current = self . current_node ( ) ; self . remove_from_stack ( & node ) ; if ! self . sink . same_node ( current , node ) {
...
local_name ! ( "li" ) => list_item_scope , _ => default_scope ,
...
self . close_p_element_in_button_scope ( ) ; if self . current_node_named ( local_name ! ( "menuitem" ) ) {
self . pop ( ) ;
} self . insert_and_pop_element_for ( tag ) ; self . frameset_ok = false ; DoneAckSelfClosing
} :: tree_builder :: types :: TagToken ( tag @ :: tokenizer :: Tag {
...
kind : :: tokenizer :: StartTag , name : local_name ! ( "menuitem" ) , ..
} ) => {
...
let current = self . current_node ( ) ; self . sink . mark_script_already_started ( current ) ;
...
if self . open_elems . len ( ) >= 2 && self . current_node_named ( local_name ! ( "option" ) ) && self . html_elem_named ( self . open_elems [ self . open_elems . len ( ) - 2 ] . clone ( ) , local_name ! ( "optgroup" ) ) {
self . pop ( ) ;
} if self . current_node_named ( local_name ! ( "optgroup" ) ) {
self . pop ( ) ;
} else {
...
matches ! ( attr . name , qualname ! ( "" , "color" ) | qualname ! ( "" , "face" ) | qualname ! ( "" , "size" ) )
} ) ; if unexpected {
...
if stack_idx == 0 {
return Done ;
} let node = self . open_elems [ stack_idx ] . clone ( ) ; let node_name = self . sink . elem_name ( node ) ; if ! first && node_name . ns == ns ! ( html ) {
